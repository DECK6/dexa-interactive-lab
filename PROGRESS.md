...
## 현재 상태 — 2026-09-05 AKM 확장

- 배포 완료: 구현 `3243729`, adxdeck `2b1b3f0` main 푸시. GitHub Pages 성공, 라이브 13카드와 배포 자산 48/48 SHA-256 일치, 메인 소개 13종 반영 확인. 배포 전 재검증: 단위 테스트 117 pass, 빌드 exit 0.
- 01 오프액시스: 고체 근경·중경·원경 가림, 고정 화면 틀, 광원·반사판 공유 장면.
- 02 핑거프레임: 0.7초 캡처 → 영상 부조 조작 → 배치(최대 5개), 기존 12 필터 유지, 모바일 설정 서랍.
- 09~13 추가: 빛의 방 / 군집 생물 / 자라는 연결망 / 웹캠 직물 / 공중 현악기. 랜딩 총 13장.
- 새 손 입력 공통 런타임: 포인터 프리뷰, 선택적 로컬 웹캠, 핀치 히스테리시스, 손 상실·카메라 거부·페이지 복원 처리.
- 최종 검증: `bun test tests/unit` 117 pass / 30,739 assertions, `bun run build` exit 0, `bun run test:e2e` 20 pass (40.4s).
- 직접 검수한 데스크톱·모바일 캡처와 명령 원문: `.planning/M3/P1/qa/`; 실행 상태: `.planning/M3/P1/STATE.json`.
- 실제 사람의 웹캠 추적 감각·음질·관객 반응은 미검증. 2026-09-05 마스터가 이번 확장분의 커밋·푸시·배포를 요청함.
- 현재 인계 기준은 `HANDOFF.md`와 이 절. 아래는 이전 세션의 역사 기록이다.
